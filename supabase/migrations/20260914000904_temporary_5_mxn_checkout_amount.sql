-- Temporary production validation amount. Existing appointments and payments are
-- intentionally left untouched; only newly acquired holds receive MXN $5.00.
...
comment on function public.acquire_booking_hold(uuid, uuid, text) is
  'Atomically acquires or replays a hold and rotates its recovery-token hash; new holds use the temporary MXN 5 production-validation amount.';
