...
export function PaymentSection({ amountMinor, currency, status: initialStatus }: {
  amountMinor: number; currency: "MXN"; status?: SafePaymentStatus;
}) {
  ...
  const handleSubmit = useCallback(async (form: { token: string; payment_method_id: string; installments: number }, additional?: { paymentTypeId?: string }) => {
    ...
    try {
      ...
      setError(code === "HOLD_EXPIRED" ? "Tu reserva expiró antes de realizar el cobro." : "No pudimos comprobar el resultado. Reintenta; no crearemos un cobro nuevo.");
