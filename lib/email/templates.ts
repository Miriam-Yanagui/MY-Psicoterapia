...
export function patientConfirmationEmail(data: AppointmentEmailData): TransactionalEmail {
  ...
  const meetAction = data.meetUrl ? `<p style="margin:18px 0 0;text-align:center"><a href="${escapeHtml(data.meetUrl)}" style="display:inline-block;padding:13px 22px;border-radius:24px;background:#1a73e8;color:#fff;text-decoration:none;font-weight:700">Entrar a la sesión en Google Meet</a></p>` : "";
...
export function practitionerNoticeEmail(data: AppointmentEmailData): TransactionalEmail {
  ...
  const calendar = data.meetUrl
    ? `<a href="${escapeHtml(data.meetUrl)}" style="color:#1a5fb4;font-weight:700">Abrir Google Meet de esta cita</a>`
    : `<strong style="color:#a33124">El evento de Calendar está ${escapeHtml(data.calendarStatus ?? "pendiente")}. Revisa la cola de Calendar.</strong>`;
