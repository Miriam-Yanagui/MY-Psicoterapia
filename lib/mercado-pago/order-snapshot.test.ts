...
const order = {
  id: "ORD-1", external_reference: "miriam-payment-1", total_amount: "5.00",
  ...
  transactions: { payments: [{ id: "PAY-1", amount: "5.00", paid_amount: "5.00",
...
describe("authoritative Order snapshot", () => {
  it("keeps only reconciliation fields and converts money exactly", () => {
    ...
    expect(snapshot?.totalAmountMinor).toBe(500);
    expect(snapshot?.payment.paidAmountMinor).toBe(500);
    expect(JSON.stringify(snapshot)).not.toContain("must-not-escape");
  });
  it.each(["5.001", "NaN", -5])("rejects invalid amounts", (total_amount) => {
